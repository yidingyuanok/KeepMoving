...
import static com.mycheering.vpf.utils.DisplayUtil.dp2px;

/**
 *通过 BlockingQueue 和 Handler 实现toast队列，不是系统级的，应用级的
...
public class MIUIToast implements IToast {
	...
	/**
	 * 原子操作：通过 判断当前是否在读取{@linkplain #mQueue 队列} 来显示toast
	 * 如果正在循环地读取Queue，就只要把toast放到Queue中就好，不需要再开启Queue
	 */
	...
	public IToast setText(String text) {
		...
		// 模拟Toast的布局文件 com.android.internal.R.activity_move.transient_notification
	...
	public void show() {
		...
		if (0 == mAtomicInteger.get()) {
			...
			mHandler.post(mActiviteRunnable);
		}
	}
	...
	public void cancel() {
		...
		if (this.equals(mQueue.peek())) {
			mHandler.removeCallbacks(mActiviteRunnable);
	...
	private static void activeQueue() {
		...
		if (miuiToast == null) {
			...
			mHandler.post(miuiToast.mShowRunnable);
			mHandler.postDelayed(miuiToast.mHideRunnable, miuiToast.mDurationMillis);
			mHandler.postDelayed(mActiviteRunnable, miuiToast.mDurationMillis);
		}
	}
	...
	private final static Runnable mActiviteRunnable = new Runnable() {
		@Override
		public void run() {
			activeQueue();
		}
